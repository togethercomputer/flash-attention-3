...
k_                    = torch.randn((batch_size, seqlen_knew, num_heads_k, head_size), dtype = dtype, device = device)
v_                    = torch.randn((batch_size, seqlen_knew, num_heads_k, head_size), dtype = dtype, device = device)
...
y = fwd_kvcache(q, kcache, vcache, k_, v_, seqlens_k_, rotary_cos_, rotary_sin_, cache_batch_idx_, leftpad_k_, 
                block_table_, alibi_slopes_, out_, softmax_scale, descale_q_, descale_k_, descale_v_, is_causal, 
                window_size_left, window_size_right, softcap, is_rotary_interleaved, num_splits, max_seqlen_k_hint, 
                use_gqa_packing)