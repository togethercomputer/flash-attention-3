        ...
        if (warp_group_idx == 0) {  // Producer
            ...
            for (auto work_tile_info = scheduler.template get_initial_work</*IsProducerWarp=*/false>(params.scheduler);
                ...
                if constexpr (Is_FP8 && !Has_softcap) {
                    float const q_descale_local = params.mainloop.ptr_q_descale == nullptr ? 1.0f : params.mainloop.ptr_q_descale[bidb];
                    float const k_descale_local = params.mainloop.ptr_k_descale == nullptr ? 1.0f : params.mainloop.ptr_k_descale[bidb];
